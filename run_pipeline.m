...
clear;
addpath(genpath('../FANTASIA-NoRMCorre'));               % add the NoRMCorre motion correction package to MATLAB path
addpath(genpath(cd))                            % added by Yueqi (6/18/18)
% gcp;                                            % start a parallel engine
foldername = 'D:\=data=\80Z_imaging\img_2p\M80Z_180905_test';
...
% ================ Run rigid motion correction (6/27 Yueqi)=============
non_rigid = false;                                 % flag for non-rigid motion correction
% ================ Output type tifff (6/27 Yueqi)=============
...
%% plottings (Yueqi 6/18/18)
i = 10;
figure,
subplot(3,1,1),plot(C_full(i,:)) 
subplot(3,1,2),plot(C_dec(i,:))
subplot(3,1,3),plot(S_dec(i,:))